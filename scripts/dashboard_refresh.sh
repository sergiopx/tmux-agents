...
if [ -z "$DASH_WIN" ]; then
  tmux display-message "tmux-openclaw: no dashboard to refresh"
  exit 1
fi
...
# Get current pane → session mapping (via @openclaw-session pane option)
...
done < <(tmux list-panes -t "$DASH_WIN" -F "#{pane_id} #{@openclaw-session}" 2>/dev/null)
...
for session in "${VISIBLE[@]}"; do
  if [ -z "${SESSION_TO_PANE[$session]}" ]; then
    ...
    tmux set-option -pt "$NEW_PANE" @openclaw-session "$session"
    tmux select-pane -T "$session" -t "$NEW_PANE"
  fi
done

# ── Reapply layout ────────────────────────────────────────────────────────────
LAYOUT=$(tmux show-option -wqv -t "$DASH_WIN" @openclaw-dashboard-layout 2>/dev/null)
LAYOUT="${LAYOUT:-tiled}"
tmux select-layout -t "$DASH_WIN" "$LAYOUT"

tmux display-message "tmux-openclaw: dashboard refreshed"
