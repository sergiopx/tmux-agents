...
MAX=$(tmux show-option -gqv "@openclaw-dashboard-max")
MAX="${MAX:-0}"
...
DEFAULT_LAYOUT=$(tmux show-option -gqv "@openclaw-dashboard-layout")
DEFAULT_LAYOUT="${DEFAULT_LAYOUT:-tiled}"

# Create window + get first pane ID
tmux new-window -n "claw:dash"
...
tmux set-option -wt "$DASH_WIN" @openclaw-dashboard 1
tmux set-option -wt "$DASH_WIN" @openclaw-dashboard-layout "$DEFAULT_LAYOUT"
...
for (( i=0; i<COUNT; i++ )); do
  ...
  tmux set-option -pt "$pane" @openclaw-session "$session"
